...
## EOD 리포트 타이밍 (장마감 직후 최대한 빠르게)

`main.py`(EOD Signal Report)는 미국 정규장 마감(16:00 ET) 정각부터 +15분
이내에만 동작하도록 뉴욕시간 기준으로 직접 판단함 (서머타임 자동 반영,
계절 상관없이 항상 "마감 후 15분 이내"가 유지됨). 15분 여유를 두는 이유는
데이터 제공처(yfinance)가 정규장 종가를 확정하는 데 몇 분 정도 걸릴 수
있어서 최소한의 안전 마진만 둔 것 - 그 이상 기다리지 않고 최대한 빨리 실행함.

워크플로우는 5분 간격으로 자주 깨어나서 이 윈도우를 놓치지 않게 하고,
하루 1번만 실제로 동작하도록 날짜 기준 중복방지 처리됨(`eod_sent.json`).
